...
- 支持两种请求方式：`Worker 后台代理` 和 `浏览器直连`。
- 支持文生图与图生图，图生图可上传多张参考图。
- 支持一次生成多张：按并发数拆成多个单图请求，完成一张展示一张。
- 支持多任务队列：任务提交后会在 Worker 后台生成，页面可以继续提交新任务。
- 支持后台任务恢复：App / 页面切到后台导致前端断流后，Worker 会继续跑任务，回到前台自动恢复结果。
...
> 4K 请求通常更慢、费用更高，建议使用 Worker 后台代理并把超时时间设置到 300-600 秒以上。
...
```text
浏览器 -> /api/tasks -> Worker 后台任务 -> 上游图片接口
```

- 推荐使用。
- 可以绕过上游 CORS 限制。
- Worker 会先创建后台任务，再由前端轮询任务状态。
- App 或浏览器切到后台导致前端连接中断时，Worker 仍会继续生成；回到前台后会自动恢复任务状态和结果。
- 多图生成时，哪一张先完成就先写入任务状态，前端恢复后可以看到已完成图片。
- 需要填写 Worker 访问密码。
- 自动上传 PiXhost 图床也通过 Worker 代理，并复用同一个 Worker 访问密码。
- 任务状态默认缓存在 Worker Cache 中约 24 小时；如果 Cloudflare 边缘缓存被清理，前端会提示任务过期，需要重新提交。
...
| `CORS` | 浏览器直连被上游拦截，建议切换到 Worker 后台代理模式。 |
...
纯 Vite 开发只跑前端，`/api/tasks`、`/api/generate-stream`、`/api/upload-pixhost` 不会生效。要完整测试 Worker：
...
- 模型：例如 `gpt-image-2`
- 请求方式：默认选 `Worker 后台代理`；如果上游支持 CORS，可以改成 `浏览器直连`
